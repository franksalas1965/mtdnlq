...
def normalize_sql(sql: str) -> str:
    """
    Corrige patrones SQL válidos en apariencia pero que fallan en PostgreSQL.

    DISTINCT + columna ::json provoca:
    «no se pudo identificar un operador de igualdad para el tipo json».
    jsonb sí admite igualdad; el cliente acepta ambos.
    """
    if re.search(r"\bDISTINCT\b", sql, re.IGNORECASE) and re.search(
        r"::json\b", sql, re.IGNORECASE
    ):
        sql = re.sub(r"::json\b", "::jsonb", sql, flags=re.IGNORECASE)
        logger.info("SQL normalizado: ::json → ::jsonb por uso de DISTINCT")

    # ILIKE '%%texto%%' (doble %) → comodines simples
    sql = re.sub(
        r"(ILIKE|LIKE)\s+'%%([^']*)%%'",
        r"\1 '%\2%'",
        sql,
...
def validate_sql(sql: str, max_results: int | None = None) -> str:
    ...
    Args:
        ...
        max_results: Límite máximo de filas a agregar si falta LIMIT. 0 = no añadir LIMIT.
