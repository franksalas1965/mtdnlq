...
async def translate_to_sql(
    ...
    Args:
        ...
        max_results: Límite de filas. 0 = todos los registros (sin LIMIT en SQL).
    ...
    if max_results == 0:
        question_for_prompt = (
            f"{question}\n"
            "[Instrucción: devolver TODOS los registros que cumplan la condición; "
            "no incluya LIMIT en el SQL.]"
        )

    prompt = build_prompt(question_for_prompt, scale=scale)
    last_error = ""
    last_sql = ""
